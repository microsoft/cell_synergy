# Thymus Embedding Generation Scripts

This directory contains scripts for generating raw H5 embedding files from finetuned thymus checkpoints.
...
- `run_embedding_{scale}_{cfg}.sbatch` - Generate embeddings for each scale/config combination
  - Scales: 0pct, 1pct, 3.16pct, 10pct, 31.6pct, 100pct
  - Configs: cfg1, cfg2, cfg3
  - Total: 18 scripts (6 scales × 3 configs)

### Submission Script
- `submit_all_thymus.sh` - Submit all embedding generation jobs

## How It Works

1. **Input**: Finetuned checkpoints from `project_folder/trained_models/thymus/scaling/{scale}/{lr_dir}/last.ckpt`
2. **Process**: Uses `cell_synergy.data.hf_dataset.generate_gex_embeddings_with_checkpoints` module
3. **Output**: Raw H5 files in `project_folder/thymus/unimodal_embeddings/gex/` (or similar)
...
# Generate Raw Embeddings Thymus
