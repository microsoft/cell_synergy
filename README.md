...
> We benchmark multimodal alignment methods for cell foundation models, focusing on the synergy between gene expression and histopathology image modalities. By extending spectral theory to cross-covariance matrices, we reveal a fundamental limitation of linear alignment methods. We introduce the Synergistic Information Score (SIS) to quantify how well methods capture non-linear interactions beyond redundancy. Our evaluation across three spatial transcriptomics datasets shows that non-spectral methods (e.g., CoMM) achieve higher synergy scores, while spectral methods (e.g., CCA, DCCA) excel at capturing linear redundancy. We demonstrate that data scaling improves alignment quality, with synergistic information being particularly important for complex spatial tasks.
