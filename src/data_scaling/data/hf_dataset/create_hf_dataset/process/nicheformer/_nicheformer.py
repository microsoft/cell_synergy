...
class Nicheformer(pl.LightningModule):
    def __init__(
        ...
        self._init_transformer(
            dim_model, nheads, dim_feedforward, nlayers, dropout, batch_first
        )

        # Embedding layers
        self._init_embeddings(
            n_tokens, dim_model, context_length, learnable_pe, dropout
        )
    ...
    def _init_embeddings(
        ...
        self.embeddings = nn.Embedding(
            num_embeddings=n_tokens + 5, embedding_dim=dim_model, padding_idx=1
        )

        if learnable_pe:
            self.positional_embedding = nn.Embedding(
                num_embeddings=context_length, embedding_dim=dim_model
            )
            ...
            self.positional_embedding = PositionalEncoding(
                d_model=dim_model, max_seq_len=context_length
            )
    ...
    def training_step(
        ...
        with torch.no_grad():
            batch = complete_masking(
                batch, self.hparams.masking_p, self.hparams.n_tokens
            )
    ...
    def _prepare_target_indices(
        self, mask: torch.Tensor, real_indices: torch.Tensor
    ) -> torch.Tensor:
    ...
    def on_before_batch_transfer(
        self, batch: tuple, dataloader_idx: int
    ) -> dict[str, torch.Tensor]:
...
def complete_masking(batch, p, n_tokens):
    ...
    mask = 1 - torch.bernoulli(
        torch.ones_like(indices), p
    )  # mask indices with probability p

    masked_indices = indices * mask  # masked_indices
    masked_indices = torch.where(
        indices != padding_token, masked_indices, indices
    ...
    random_tokens = random_tokens * torch.bernoulli(
        torch.ones_like(random_tokens) * 0.1
    ).type(torch.int64)
    ...
    same_tokens = same_tokens * torch.bernoulli(
        torch.ones_like(same_tokens) * 0.1
    ).type(torch.int64)
