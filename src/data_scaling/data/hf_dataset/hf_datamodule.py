...
class SimpleHFDataset(Dataset):
    ...
    def __init__(self, hf_dataset, sample_names: Optional[List[str]] = None):
        ...
        if sample_names is not None:
            ...
            logger.info(f"Filtered dataset from {original_size} to {len(self.dataset)} samples")
    ...
    def __getitem__(self, idx):
        ...
        if not hasattr(self, '_debug_logged'):
            logger.info(f"Raw gexp data type: {type(gexp_raw)}")
            logger.info(f"Raw gexp shape: {gexp_raw.shape if hasattr(gexp_raw, 'shape') else 'N/A'}")
            logger.info(f"Raw gexp dtype: {gexp_raw.dtype if hasattr(gexp_raw, 'dtype') else 'N/A'}")
            if hasattr(gexp_raw, 'shape') and len(gexp_raw.shape) > 0:
                logger.info(f"Raw gexp value range: min={gexp_raw.min()}, max={gexp_raw.max()}")
        ...
        if gexp_tensor.dim() == 3:
            ...
            logger.info(f"Processing 3D tensor with shape: {gexp_tensor.shape}")
            ...
            logger.info(f"Processing 2D tensor with shape: {gexp_tensor.shape}")
            gexp_tokens = torch.argmax(gexp_tensor, dim=-1).long()  # [seq_len]
            
        elif gexp_tensor.dim() == 1:
            # Already token indices, just ensure correct dtype
            logger.info(f"Processing 1D tensor with shape: {gexp_tensor.shape}")
        ...
        if gexp_tokens.min() < 0:
            logger.warning(f"Found negative token values: min={gexp_tokens.min()}. Converting to absolute values.")
        ...
        if 'mask' in data:
            ...
            if mask.shape[0] != gexp_tokens.shape[0]:
                logger.warning(f"Mask length {mask.shape[0]} doesn't match sequence length {gexp_tokens.shape[0]}. Creating new mask.")
        ...
        if not hasattr(self, '_final_debug_logged'):
            logger.info(f"Final gexp_tokens shape: {gexp_tokens.shape}, dtype: {gexp_tokens.dtype}")
            logger.info(f"Final mask shape: {mask.shape}, dtype: {mask.dtype}")
            logger.info(f"Token value range: min={gexp_tokens.min()}, max={gexp_tokens.max()}")
            logger.info(f"Sample token values: {gexp_tokens[:10] if len(gexp_tokens) > 0 else 'Empty'}")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.dataset is None:
            ...
            try:
                ...
                logger.info(f"Loaded dataset with simplified approach: {len(self.dataset)} samples")
