...
def analyze_donors(donor_ids, donor_sex):
    total = len(donor_ids)
...
def select_donors(donor_ids, donor_sex, n_donors=None, remove_n_female=None, only_sex=None, seed=42):
    """Return set of donor_ids to keep based on selection criteria."""
    np.random.seed(seed)
...
def filter_and_write(source_dir, output_dir, keep_donors):
...
def main():
    ...
    # Analyze mode
    parser_analyze = subparsers.add_parser('analyze', help='Analyze donor composition in the dataset')
