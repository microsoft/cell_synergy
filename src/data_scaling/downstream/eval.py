...
def load_split_data(cfg: DictConfig, split_name, selected_method=None):
    ...
    if cfg.evaluation.tasks.regress and 'cell_types' in cfg.data:
        if 'nomap_index' in cfg.data.cell_types:
            ...
            print("Removed NOMAP from cell type ratios and renormalized")
    
    print(f"Final dataset size: {len(ds)} samples")

    return ds

def evaluate_simple(X_train, X_test, y_train, y_test, train_reg_labels=None, test_reg_labels=None, random_state=42, cfg=None):
    """
...
def run_embedding_baseline_eval(cfg: DictConfig, train_ds, test_ds, selected_method):
...
def main(cfg: DictConfig):
    ...
    print(f"  - Selected method: {selected_method}")
    print(f"  - Evaluation strategy: Pretrain-Test (entire splits)")
    ...
    # Run evaluation
    result = run_embedding_baseline_eval(cfg, train_ds, test_ds, selected_method)
