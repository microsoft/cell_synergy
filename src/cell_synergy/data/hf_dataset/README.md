...
This module provides PyTorch Lightning DataModules for working with HuggingFace datasets in the cell_synergy project.

## Required Data Files

The following data files are required for Nicheformer embedding generation. These should be downloaded from the original Nicheformer publication repository and placed in this directory (`src/cell_synergy/data/hf_dataset/`):

1. **`xenium_mean_script.npy`**: Technology-specific mean expression values for Xenium data
   - Used by: `tokenize_hf_dataset.py`, `generate_gex_embeddings_with_checkpoints.py`
   - Purpose: Normalization values for gene expression data

2. **`nicheformer_reference.h5ad`**: Reference AnnData object with gene names
   - Used by: NicheformerEmbedder (for gene name mapping and reference)
   - Purpose: Provides gene name mappings and reference structure for Nicheformer

**Note**: These files are not included in this repository due to size and licensing. Please refer to the original Nicheformer publication for download instructions.
...
```python
from cell_synergy.data.hf_dataset import HFDataModule
...
```python
from cell_synergy.data.hf_dataset import create_hf_datamodule_from_config
...
```python
from cell_synergy.data.hf_dataset import HFDataModuleWithCollate
...
from cell_synergy.data.hf_dataset.hf_datamodule import HFDataModule
