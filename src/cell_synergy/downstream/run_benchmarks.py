...
from cell_synergy.data.dataset_preparation import prepare_donor_tensor_datasets
from cell_synergy.downstream.eval import make_serializable
from cell_synergy.downstream.linear_probe import train_linear_probe
from cell_synergy.downstream.spatial_neighbor_distance_bins import run_spatial_neighbor
...
def get_project_dir() -> Path:
    # Use PROJECT_DIR from paths module
    ...
    for donor, samples in donor_datasets.items():
        ...
        try:
            ...
            if modality == "multimodal":
                ...
                labels = torch.tensor([s[label_key] if isinstance(s[label_key], (int, float))
                                      else np.argmax(s[label_key]) for s in samples])
                ...
                if cfg.evaluation.tasks.regress:
                    ...
                    if regression_key in samples[0]:
                        # Use the entire cell type ratio vector, not just the first element
...
def run_unimodal_baselines(cfg: DictConfig) -> Dict[str, float]:
    ...
    # donor_splits = get_donor_splits(donor_ids)  # Unused, kept for potential future use
...
def aggregate_metrics(metrics_list, explained_var=None, cumulative_var=None):
    # metrics_list: list of dicts with keys train_accuracy, train_f1_macro,
    # accuracy, f1_macro, train_r2, train_mse, r2, mse
    keys = [
